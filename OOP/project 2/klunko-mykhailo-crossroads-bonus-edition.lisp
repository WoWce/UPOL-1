...
(defvar *program*
  '((t nil t) (t t nil) (t t nil) (t nil t)))

(defvar *default-program*
  '((t nil) (t t) (t t) (t nil)))

(defun prog-sem-num ()
  (length (first *program*)))
...
(defun 3sem ()
  (let ((top-l (make-instance 'polygon))
        ...
        (crosswalk3 (make-instance 'crosswalk))
        (sem-v (make-instance 'semaphore))
        (sem-v2 (make-instance 'semaphore))
        (sem-p (make-instance 'semaphore)))
    (set-items top-l (make-crossroad '((250 20) (250 250) (20 250))))
    (set-items top-r (make-crossroad '((350 20) (350 250) (580 250))))
    (set-items bot-l (make-crossroad '((20 350) (250 350) (250 580))))
    ...
    (set-semaphore-type sem-v :vehicle)
    (set-semaphore-type sem-p :pedastrian)
    (set-semaphore-type sem-v2 :vehicle)
    ;vehicle
    (move sem-v 351 351)
    ;pedastrian
    (move sem-p 90 160)
    ...
    (set-color bot-l :black)
    (set-color bot-r :black)
    (list sem-v sem-p sem-v2 top-l top-r bot-l bot-r crosswalk1 crosswalk2 crosswalk3)))
...
(defmethod set-crossroads-phase ((c crossroads) value)
  (if (= value 0)
      (dotimes (x (phase-len c)) 
            (set-semaphore-phase (nth x (items c)) 0))
    (dotimes (x (phase-len c))
...
(defmethod sem-count ((c crossroads))
  (length (semaphores c)))

(defun create-default-prog (c)
  (let ((x))
    (dotimes (j (phase-count c))
      (setf x (cons
               (let ((y)) (dotimes (i (sem-count c))
                            (setf y (cons nil y))
                            )y)
               x))) 
    (reverse x)))

(defun add-to-prog (program)
  (let ((a)) (dolist (x program) ;slot-value c 'program
               (setf a (cons (append x '(t)) a))) (reverse a)))

(defun delete-from-prog (program)
  (let ((a)) (dolist (x program) ;slot-value c 'program
               (setf a (cons (reverse(cdr (reverse x))) a))) (reverse a)))

(defun do-add (program count)
  (let ((a program))
    (dotimes (x count)
      (setf a (add-to-prog a)))
    a))

(defun do-delete (program count)
  (let ((a program))
    (dotimes (x count)
      (setf a (delete-from-prog a)))
    a))

(defmethod add-to-sem-prog ((c crossroads) count)
  (set-program c (do-add (slot-value c 'program) count)))

(defmethod del-from-sem-prog ((c crossroads) count)
  (set-program c (do-delete (slot-value c 'program) count)))
  

(defmethod prog-sem-count ((c crossroads))
  (length (first (slot-value c 'program))))
...
;
;
(defmethod set-items ((c crossroads) list)
  ;(setf (slot-value c 'items) list)
  (call-next-method)
  ;;put semaphore objects to semaphores slot
  (let ((a))
               (dolist (x (items c))
                 (if (eql 'semaphore (type-of x))
                     (setf a (cons x a))))
               (setf (slot-value c 'semaphores) (reverse a)))
  ;;edit/add program
  (cond 
   ((not (slot-value c 'program)) (set-program c (create-default-prog c)))
   ((> (sem-count c) (prog-sem-count c)) (add-to-sem-prog c (- (sem-count c) (prog-sem-count c))))
   ((< (sem-count c) (prog-sem-count c)) (del-from-sem-prog c (- (prog-sem-count c) (sem-count c)))))
  (set-crossroads-phase c 0)) ;;add semaphore
...
(defmethod set-program ((c crossroads) prog-list)
  (if (semaphores c)
      (dolist (x prog-list)
        (unless (= (length x) (length (slot-value c 'semaphores)))
          (error "Wrong format of crossroads program"))))
